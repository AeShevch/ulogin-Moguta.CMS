...
/*
  Plugin Name: uLogin
  Description: uLogin.ru - виджет авторизации через социальные сети.
...
class ULoginAuth
{
    ...
    public function __construct()
    {
        ...
        $meta .= '<link href="' . SITE . '/' . self::$path . '/css/style.css" rel="stylesheet" type="text/css">';
        mgAddMeta($meta);
    }
    ...
    static function getPanelCode($place = 0)
    {
        ...
        $ulogin_default_options['providers'] = 'vkontakte,google,yandex,facebook';
    ...
    static function getSyncPanelCode($user_id = 0)
    {
        ...
        <section class="ul-social-sync">
            <div class="ul-social-sync__item ul-social-sync__item_add ul-add-account">
                <h2 class="ul-social-sync__title ul-add-account__title">
                    <?php echo self::$lang['ULOGIN_SYNC']; ?>
                </h2>
                <p class="ul-social-sync__subtitle">
                    <?php echo self::$lang['ULOGIN_SYNC_HELP']; ?>
                </p>
                <?php echo self::getPanelCode(1); ?>
            </div>

            <?php if (!empty($networks)): ?>
                <div class="ul-social-sync__item ul-social-sync__item_added ul-added-socials">
                    <h2 class="ul-social-sync__title">
                        <?php echo self::$lang['ULOGIN_SYNC_LIST']; ?>
                    </h2>
                    <p class="ul-social-sync__subtitle">
                        <?php echo self::$lang['ULOGIN_SYNC_DELETE']; ?>
                    </p>

                    <div id="ulogin_accounts"
                         class="ul-added-socials__inner">
                        <?php foreach ($networks as $network) {
                            if ($network['user_id'] = $user_id) { ?>
                                <div data-ulogin-network="<?php echo $network['network']; ?>"
                                     data-ulogin-identity="<?php echo $network['identity']; ?>"
                                     title="<?php echo self::$lang['ULOGIN_SYNC_DELETE_ITEM']; ?> <?php echo $network['network']; ?>"
                                     class="ul-added-socials__item ulogin_network big_provider <?php echo $network['network']; ?>_big"></div>
                            <?php }
                        } ?>
                    </div>
